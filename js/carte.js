document.addEventListener('DOMContentLoaded', function () {
    ...
    function construireCartePlat(plat) {
        ...
        return `
            <article class="dish-card"
                     data-prix="${plat.prix}"
                     data-type="${(plat.type || '').toLowerCase()}">
                <div class="dish-head">
    ...
    function trierArticles(grille, critere) {
        ...
        articles.forEach(article => grille.appendChild(article));
    }
    ...
    if (formulaireFiltres) {
        formulaireFiltres.querySelectorAll('input[type="checkbox"]').forEach(cb =>
        ...
        if (boutonReset) boutonReset.addEventListener('click', () => setTimeout(appliquerFiltres, 50));
    }
