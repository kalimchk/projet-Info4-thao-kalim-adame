document.addEventListener('DOMContentLoaded', function () {
    ...
    const sectionsCommandes = document.querySelectorAll('.commande-modifiable');

    sectionsCommandes.forEach(function (section) {
        ...
        function mettreAJourTotal(montant) {
            if (totalAffiche)
                totalAffiche.textContent = 'Total : ' + montant.toFixed(2).replace('.', ',') + ' €';
        }
        ...
        async function modifierCommande(nomProduit, prixUnitaire, typeAction) {
            try {
                ...
                if (!resultat.succes) { afficherMessage('❌ ' + resultat.message, 'erreur'); return; }

                reconstruireListeArticles(resultat.articles);
                mettreAJourTotal(resultat.nouveau_montant);

                if (resultat.paiement_requis) {
                    afficherMessage(resultat.message, 'avertissement');
        ...
        function attacherEcouteursBoutons() {
            listeArticles.querySelectorAll('.btn-retirer-article').forEach(btn =>
                btn.addEventListener('click', () => modifierCommande(btn.dataset.nom, parseFloat(btn.dataset.prix), 'retirer'))
            );
            listeArticles.querySelectorAll('.btn-ajouter-article').forEach(btn =>
                btn.addEventListener('click', () => modifierCommande(btn.dataset.nom, parseFloat(btn.dataset.prix), 'ajouter'))
            );
        }
